...
function Rule.__tostring (rule)
  ...
  return table.concat (t, " = ")
      .. (rule.when and " when " .. tostring (rule.when) or "")
end
...
function Strategy.__call (strategy, term)
  ...
  assert (term == nil or getmetatable (term) == Term or getmetatable (term) == Variable,
          "term must be a term or a variable")
  if term == nil then
    return nil
  else
...
function Strategy.rule (rule)
  ...
  return Strategy (function (term)
    ...
    if ok then
      if rule.when then
        local evaluation = Strategy.innermost (rule.when)
        if evaluation (rule.when / mapping) ~= Boolean.True {} then
          return nil
        end
      end
